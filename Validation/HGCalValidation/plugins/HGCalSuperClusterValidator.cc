...
void HGCalSuperClusterValidator::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<double>("simToRecoScoreThreshold_forEfficiency", 0.3)
    ...
    desc.add<double>("simToRecoScoreThreshold_forResolution", 0.9)
      ->setComment(
          "Threshold on the Sim->Reco score between SimTrackster from CaloParticle and Supercluster trackster "
          "collection to consider a supercluster for energy scale and resolution plots (typically a looseer cut than for efficiency, otherwise the energy scale plots are one-sided)");
...
void HGCalSuperClusterValidator::bookHistograms(DQMStore::IBooker& ibook,
  ...
  histos.sim_ET_vs_Eta_num = ibook.book2D("sim_ET_vs_Eta_num",
                                          std::string("SuperClusters in HGCAL pt-abs(eta) (matched to CaloParticle, reco2sim<")+simToRecoScoreThreshold_forEfficiency_+")",
                                          ptBins.size() - 1,
                                          ptBins.data(),
                                          etaBins.size() - 1,
                                          etaBins.data());
  histos.sim_ET_vs_Eta_num->setXTitle("CaloParticle regressed energy (ie sum of sim energy of electron/photon in calorimeters) [GeV]");
  histos.sim_ET_vs_Eta_denom = ibook.book2D("sim_ET_vs_Eta_denom",
                                            ...
                                            etaBins.size() - 1,
                                            etaBins.data());
  histos.sim_ET_vs_Eta_denom->setXTitle("CaloParticle regressed energy (ie sum of sim energy of electron/photon in calorimeters) [GeV]");
  ...
  histos.sc_EoverEtruth_vs_ET->setXTitle("Transverse energy from CaloParticle (ie sum of sim transverse energy of electron/photon in calorimeters) [GeV]");
  histos.sc_EoverEtruth_vs_ET->setYTitle("Reconstructed raw energy of supercluster / energy from CaloParticle (Ereco/Etrue)");
  ...
  histos.sc_EoverEtruth_vs_ET_lowEta = ibook.book2D("sc_EoverEtruth_vs_ET_lowEta",
                                                    ...
                                                    EoverEtruthBins.size() - 1,
                                                    EoverEtruthBins.data());
  histos.sc_EoverEtruth_vs_ET_lowEta->setXTitle("Transverse energy from CaloParticle (ie sum of sim transverse energy of electron/photon in calorimeters) [GeV]");
  histos.sc_EoverEtruth_vs_ET_lowEta->setYTitle("Reconstructed raw energy of supercluster / energy from CaloParticle (Ereco/Etrue)");
  ...
  histos.sc_EoverEtruth_vs_ET_midEta =
      ibook.book2D("sc_EoverEtruth_vs_ET_midEta",
                   ...
                   EoverEtruthBins.size() - 1,
                   EoverEtruthBins.data());
  histos.sc_EoverEtruth_vs_ET_midEta->setXTitle("Transverse energy from CaloParticle (ie sum of sim transverse energy of electron/photon in calorimeters) [GeV]");
  histos.sc_EoverEtruth_vs_ET_midEta->setYTitle("Reconstructed raw energy of supercluster / energy from CaloParticle (Ereco/Etrue)");
  ...
  histos.sc_EoverEtruth_vs_ET_highEta = ibook.book2D("sc_EoverEtruth_vs_ET_highEta",
                                                     ...
                                                     EoverEtruthBins.size() - 1,
                                                     EoverEtruthBins.data());
  histos.sc_EoverEtruth_vs_ET_highEta->setXTitle("Transverse energy from CaloParticle (ie sum of sim transverse energy of electron/photon in calorimeters) [GeV]");
  histos.sc_EoverEtruth_vs_ET_highEta->setYTitle("Reconstructed raw energy of supercluster / energy from CaloParticle (Ereco/Etrue)");
  histos.sc_EoverEtruth_vs_eta = ibook.book2D("sc_EoverEtruth_vs_eta",
                                              ...
                                              EoverEtruthBins.size() - 1,
                                              EoverEtruthBins.data());
  histos.sc_EoverEtruth_vs_eta->setXTitle("Reconstructed raw energy of supercluster / energy from CaloParticle (Ereco/Etrue)");
}
