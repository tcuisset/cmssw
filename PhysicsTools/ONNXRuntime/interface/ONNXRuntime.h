...
namespace cms::Ort {
  ...
  struct TensorArgsBool : public TensorArgs<uint8_t> {
  ...
  template<typename TensorArgsT>
  void runIntoVariable(
                TensorArgsT inputArgs,
                 const std::vector<std::string>& output_names,
                 FloatArrays& output_values,
                 const std::vector<std::vector<int64_t>>& output_shapes,
                 int64_t batch_size) const {
                      assert(output_shapes.empty() || (!output_names.empty() && output_shapes.size() == output_names.size()));
    static_assert(std::tuple_size<TensorArgsT>{}>0, "inputArgs must be a std::tuple");
    assert(batch_size > 0);
    ...
    // The following labmda is to loop over inputArgs
    auto create_tensor_input = [&]<typename T>(T& args) {
      ...
      if (expected_len != static_cast<int64_t>(args.input_value.size())) {
        throw cms::Exception("RuntimeError")
            << "Input array " << args.input_name << " has a wrong size of " << args.input_value.size() << ", expected " << expected_len;
      }

      auto input_tensor = // float
          ::Ort::Value::CreateTensor<typename T::onnx_type>(memory_info, args.convertToPointer(), args.input_value.size(), input_dims.data(), input_dims.size());
      assert(input_tensor.IsTensor());
      input_tensors.emplace_back(std::move(input_tensor));
    };
    std::apply([&](auto&&... args) {((create_tensor_input(args)), ...);}, inputArgs);
    ...
    for (size_t i = 0; i < resolved_output_names.size(); ++i) {
      ...
      auto out_tensor =
          ::Ort::Value::CreateTensor<float>(memory_info, out_buf.data(), out_buf.size(), out_dims.data(), out_dims.size());
      assert(out_tensor.IsTensor());
