...
Authors : Theo Cuisset <theo.cuisset@polytechnique.edu>, Shamik Ghosh <shamik.ghosh@cern.ch>
Date : 11/2023
*/
...
TracksterLinkingbySuperClustering::TracksterLinkingbySuperClustering(const edm::ParameterSet& ps,
  ...
  assert(onnxRuntime_ &&
         "TracksterLinkingbySuperClustering : ONNXRuntime was not provided, the model should have been set in "
...
*/
void TracksterLinkingbySuperClustering::linkTracksters(
    const Inputs& input,
  ...
  */
        ...
        for (unsigned int ts_seed_idx_pt :
          ...
          if (std::abs(ts_seed.barycenter().Eta() - ts_cand.barycenter().Eta()) < deltaEtaWindow_ &&
            ...
            if (candidateIndexInCurrentBatch >= miniBatchSize) {
              ...
              TODO In case memory usage is a problem, std::vector::reserve could be used with a more optimistic estimate
            ...
            std::copy(features.begin(),
                      features.end(),
                      inputTensorBatches.back().begin() + candidateIndexInCurrentBatch * nnInput->featureCount());
  ...
  if (!inputTensorBatches.empty()) {
    ...
    for (unsigned int i = 0;
        i < std::min(nnInput->featureCount() * 20, static_cast<unsigned int>(inputTensorBatches[0].size()));
        i++) {
  ...
  for (std::vector<float>& singleBatch : inputTensorBatches) {
    ...
    std::vector<float> outputs =
        onnxRuntime_->run({"input"}, inputs_for_onnx, {}, {}, inputs_for_onnx[0].size() / nnInput->featureCount())[0];
